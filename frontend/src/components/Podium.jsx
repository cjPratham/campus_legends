...
export default function Podium({ students }) {
  const getRankChangeIcon = (change) => {
    if (change > 0) return <span className="text-green-600 ml-1">⬆</span>;
    if (change < 0) return <span className="text-red-600 ml-1">⬇</span>;
  ...
  return (
    <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-4 sm:gap-6 mb-12 w-full">
      {students.map((s, i) => (
        ...
        >
          {/* Rank & Rank Change */}
          <div className="flex items-center justify-center text-lg sm:text-2xl font-bold">
            {i + 1} {i === 0 && "👑"} {getRankChangeIcon(s.rankChange)}
          </div>
          ...
          <p className="text-lg sm:text-xl font-bold mt-2">
            {s.totalPoint.toLocaleString()}{getPointsChangeText(s.pointsChange)}
          </p>
        </motion.div>
      ))}
    </div>
